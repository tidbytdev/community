...
Summary: Literature Clock
Description: Displays literature quotes for current time.
Author: tidbytdev
"""
...
def fetch_hour_data(hour_str, now):
    ...
    if response.status_code != 200:
        print("Failed fetch: %s, Status: %d" % (url, response.status_code))
        return None
    body = response.body()
    if not body:
        print("Empty response body for: %s" % url)
        return None
    data = json.decode(body)
    if data == None:
        fail("Failed to decode JSON for: %s" % url)
    return data
...
def get_quote_for_minute(hour_data, minute_str):
    ...
    if not hour_data or minute_str not in hour_data:
        return None
    quotes = hour_data.get(minute_str)
    if not quotes or type(quotes) != "list":
        return None
...
def prepare_quote_lines(quote, quote_color, author_color, title_color, title_display_font, author_display_font):
    ...
    left = replace_br(quote.get("left", "") or "")
    bold = replace_br(quote.get("bold", "") or "")
    right = replace_br(quote.get("right", "") or "")
    author = replace_br(quote.get("author", "") or "")
    title = replace_br(quote.get("title", "") or "")
    main_quote_parts = {"left": left, "bold": bold, "right": right}
    meta_lines = []
    if (left or bold or right) and (title or author):
        meta_lines.append({"text": " ", "color": quote_color, "font": author_display_font, "align": "center"})
    if title:
        meta_lines.append({"text": title, "color": title_color, "align": "center", "font": title_display_font})
    if author:
        meta_lines.append({"text": author, "color": author_color, "align": "center", "font": author_display_font})
...
def render_scrolling_quote(
    ...
    main_quote_parts, meta_lines = prepare_quote_lines(
        ...
        title_display_font = title_font,
        author_display_font = author_font,
    )
    has_left = bool(main_quote_parts["left"])
    has_bold = bool(main_quote_parts["bold"])
    has_right = bool(main_quote_parts["right"])
    has_main_quote = has_left or has_bold or has_right
    has_meta = bool(meta_lines)
    if not has_main_quote and not has_meta:
        return render_error(quote_color, bg_color, quote_before_font)

    start_pos = 32
    fixed_scroll_height = 32 * 5
    end_pos = -(fixed_scroll_height)
    loop_end = end_pos - 1
...
def main(config):
    ...
    quote_color = config.get("quote_color", "#FFFFFF")
    time_ref_color = config.get("time_ref_color", "#FFDB5B")
    author_color = config.get("author_color", "#95FF8A")
    title_color = config.get("title_color", "#8AFFD4")
    bg_color = config.get("bg_color", "#000000")
    quote_before_font_name = config.get("quote_before_font", "tb")
    quote_before_font_size = config.get("quote_before_font_size", "small")
    time_ref_font_name = config.get("time_ref_font", "6x13")
    time_ref_font_size = config.get("time_ref_font_size", "large")
    quote_after_font_name = config.get("quote_after_font", "tb")
    quote_after_font_size = config.get("quote_after_font_size", "small")
    title_font_name = config.get("title_font", "tb")
    title_font_size = config.get("title_font_size", "small")
    author_font_name = config.get("author_font", "tb")
    author_font_size = config.get("author_font_size", "small")
    ...
    # --- Timezone Logic ---
    user_tz_selection = config.get("user_timezone", "$tz")
    device_tz = config.get("$tz")
    default_tz = "America/New_York"
    tz = default_tz
    if user_tz_selection and user_tz_selection != "$tz":
        tz = user_tz_selection
    elif device_tz:
        tz = device_tz
    ...
    if not current_hour_data:
        return render_error(quote_color, bg_color, quote_before_font)  # Use quote_before font
    ...
    return render_scrolling_quote(
        quote = quote,
        quote_color = quote_color,
        time_ref_color = time_ref_color,
        author_color = author_color,
        title_color = title_color,
        bg_color = bg_color,
        quote_before_font = quote_before_font,
        time_ref_font = time_ref_font,
        quote_after_font = quote_after_font,
